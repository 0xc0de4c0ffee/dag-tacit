...
*dag-tacit (this repo): Indexer classification MUST match **§5** (envelope script + opcode payloads `0x21`–`0x2C`, including **§5.12–§5.13** `T_DROP` / `T_DCLAIM`). Implementation: `indexer/lib/decode.ts` (`decodeEnvelopeScript`, `decodePayload`). Deployment floor for **which heights enter the tacit index** (separate from protocol validity): env **`TACIT_INDEX_MIN_HEIGHT`** (CLI/server; **948942** for current mainnet tacitscan genesis) — see [PROCESS.md](./PROCESS.md). Bitcoin Core RPC JSON uses **BTC floats** for `value` / `fee` / `prevout.value`; the DAG stores **`uint` satoshis** per [SPEC.md](./SPEC.md). CAR transport: [PROCESS.md](./PROCESS.md). Normative Tacit protocol text: [github.com/z0r0z/tacit `SPEC.md`](https://github.com/z0r0z/tacit/blob/main/SPEC.md).*
